...
def formarTabla():
    for i in range(filas):
        # La "y" de la primer fila la formamos por separado
...
def imprimirResultados():
    ...
    for i in range(longitud):
        if(str(R[i]) != "a"):
            File.write("y"+str(i)+": "+str(y[i])+"\t"+str(ceros[i])+"\t\tx"+str(i+1)+": "+str(x[i])+"\tr"+str(i+1)+": "+str(R[i])+"\n")
    File.write("================================ VALORES DUPLICADOS =================================\n")
    for i in range(longitud2):
        if(str(duplicados[i]) != "a"):
            if(len(str(y[i])) < 6):
                File.write("y"+str(i)+": "+str(y[i])+"\t\t\t"+str(ceros[i])+"\t\tx"+str(i+1)+": "+str(x[i])+"\tr"+str(i+1)+": "+str(duplicados[i])+"\n")
            else:
                File.write("y"+str(i)+": "+str(y[i])+"\t"+str(ceros[i])+"\t\tx"+str(i+1)+": "+str(x[i])+"\tr"+str(i+1)+": "+str(duplicados[i])+"\n")
    File.close()
